...
import { katakanaData } from './katakana_text_data.js';
...
document.addEventListener("DOMContentLoaded", function() {
    const gameState = {
        ...
        streak: 0,
        bestStreak: parseInt(localStorage.getItem("katakana_bestStreak")) || 0,
        incorrectCount: 0,
        isFirstAttempt: true,
        humorMode: false,
        wrongCounts: JSON.parse(localStorage.getItem('katakanaTextWrongCounts') || '{}'),
        currentComboKey: ""
    };
    ...
    const elements = {
        ...
        katakanaDisp: document.getElementById("katakana-display"),
    ...
    elements.startQuizBtn.addEventListener("click", () => {
        ...
        checked.forEach(key => {
            if (katakanaData[key]) gameState.selectedCharacters.push(...katakanaData[key]);
        });
    ...
    function saveWrongCounts() {
        localStorage.setItem('katakanaTextWrongCounts', JSON.stringify(gameState.wrongCounts));
    }
});